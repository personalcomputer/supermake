...
libraries = { #There are a lot of problems with the current approach, but most are inherit to Supermake's design, unfortunately. I just use the most common installation setups (specifically: whatever ubuntu uses) for these libraries.
  ...
  'cln': ['-lcln'],
  'OIS/': ['-lOIS'],
  'gtk/': ['`pkg-config --cflags --libs gtk+-3.0`'], #No 2.x support. Supermake simply is not capable or designed to support multiple library versions. (but it will only take changing a single character if you need it..)
...
class CodeFile:
  ...
  def __init__(self, filepath, codeFilesStore):
    ...
    m = re.findall(r'^#include <(.+?)>', self._content, re.MULTILINE) #Warning, if #includes are commented out using C comments(/* */), they will still be considered included.  I don't know how to avoid this short of implementing a pre-processor. use cpp maybe?
    for header in m:
