...
#There are a lot of problems with the current approach, but this is inherit to Supermake's design, unfortunately. I just use the most common instalation setups (specifically, whatever ubuntu uses ;)) for these libraries.
libraries = {}
...
#These are only file extensions that are unique to the languages.
c_source_extensions   = ['c']
cpp_source_extensions = ['c++', 'cc', 'cpp', 'cxx']
cpp_header_extensions = ['h++', 'hh', 'hpp', 'hxx']

source_extensions = c_source_extensions
source_extensions.extend(cpp_source_extensions) #...Really need a way to extend a rvalue list (immutable)...

header_extensions = cpp_header_extensions
...
def sourceCodeInDirectory(): #DOES NOT INCLUDE HEADERS
  ...
  if not sourceCodeInDirectoryCache:
    sourceCodeInDirectoryCache = sorted([filename for filename in os.listdir('.') if ((not os.path.isdir(filename)) and splitOnExtension(filename)[1] in source_extensions)])
...
def getFileDeps(filename, maxrecurse): #takes a source file and finds all #includes(recursively) in order to determine what files the source-file depends upon
  global depend #you so silly python
  maxrecurse -= 1
  if(maxrecurse < 0):
    emptylist = []
    return emptylist
  ...
  m = re.findall(r'^#include <(.+)>', GetFileFromCache(filename), re.MULTILINE) #Warning, if #includes are commented out using C comments(/* */), they will still be included.  I don't know how to avoid this using regex, probably not possible with just regex. TODO
  for case in m:
    if case in libraries:
      depend.extend(libraries[case])
  m = re.findall(r'^#include "(.+)"', GetFileFromCache(filename), re.MULTILINE)
  deps = []
  for case in m:
    if splitOnExtension(case)[1] not in header_extensions:
      message('Warning: Found unknown included file \''+case+'\'.')
      
      #this should be done as an exception but I don't do exceptions in python, I treat/use it like a simple scripting language... Time to port this terrible code to C++.
      deps.append(os.path.join(os.path.split(filename)[0], os.path.normpath(case)))
      deps = list(set(deps)) 
...
def BinaryGuessingStrategy_ContainingProjectFolderName():
  ...
  if os.path.basename(os.getcwd()) != 'src' and os.path.basename(os.getcwd()) != 'source':
    return False
...
def BinaryGuessingStrategy_RootClassName():
  #Guessing Strategy: Look for the main() function and see if it is in the top-level application class file, which is semi common, but moreso in java than C++.
  #TODO
  return False
...
def main():
  ...
  projectLanguage = 'unknown' #using strings because python lacks enums. valid values: unknown, c, c++.
  hasSourceFiles = False
  if len(sourceCodeInDirectory()) <= 0:
    message('Error: No sourcecode found. For help see --help.', critical=True)
    sys.exit()
  for filename in sourceCodeInDirectory(): 
    if splitOnExtension(filename)[1] in cpp_source_extensions: #Language is determined entirely by file extension.
      projectLanguage = 'c++'
    elif splitOnExtension(filename)[1] in c_source_extensions:
      if projectLanguage == 'unknown': #Meaning, if it has c++ and c mixed, we assuume c++
        projectLanguage = 'c'
  ...
  compiler = ''
  assert projectLanguage != 'unknown'
  if projectLanguage == 'c':
    compiler = 'gcc'
  elif projectLanguage == 'c++':
    compiler = 'g++'
